...
static int rocblas_init(int device_id, void* lib_handle, void** backend_handle) {
    /* Note: lib_handle is unused for rocBLAS - HIP libraries are statically linked */
    (void)lib_handle;  /* Suppress unused parameter warning */
    
    hipError_t hip_err;
    rocblas_status blas_status;
    rocblas_status solver_status;
    
    // Set device
    hip_err = hipSetDevice(device_id);
    if (hip_err != hipSuccess) {
        fprintf(stderr, "rocBLAS: Failed to set device %d: %s\n",
                device_id, hipGetErrorString(hip_err));
        return -1;
    }
    ...
    blas_status = rocblas_create_handle(&ctx->blas_handle);
    ...
    solver_status = rocblas_create_handle(&ctx->solver_handle);
...
static int rocblas_convert_uplo(char uplo) {
    switch (uplo) {
        case 'U': case 'u': return rocblas_fill_upper;
        case 'L': case 'l': return rocblas_fill_lower;
        default:
...
static int rocblas_convert_diag(char diag) {
    switch (diag) {
        case 'N': case 'n': return rocblas_diagonal_non_unit;
        case 'U': case 'u': return rocblas_diagonal_unit;
        default:
...
static void rocblas_chemv_impl(void* handle, fb_gpu_stream_t stream, char uplo,
                               ...
                               fb_gpu_ptr_t y, int incy) {
    rocblas_context_t* ctx = (rocblas_context_t*)handle;
    rocblas_fill fill = (rocblas_fill)rocblas_convert_uplo(uplo);
...
static void rocblas_strmv_impl(void* handle, fb_gpu_stream_t stream,
    ...
    rocblas_fill fill = (rocblas_fill)rocblas_convert_uplo(uplo);
    rocblas_operation op = (rocblas_operation)rocblas_convert_transpose(trans);
    rocblas_diagonal diag_type = (rocblas_diagonal)rocblas_convert_diag(diag);
...
static void rocblas_stbmv_impl(void* handle, fb_gpu_stream_t stream,
    ...
    rocblas_fill fill = (rocblas_fill)rocblas_convert_uplo(uplo);
    rocblas_operation op = (rocblas_operation)rocblas_convert_transpose(trans);
    rocblas_diagonal diagtype = (rocblas_diagonal)rocblas_convert_diag(diag);
    if (stream) rocblas_set_stream(ctx->blas_handle, (hipStream_t)stream);
...
static void rocblas_sgemm_impl(void* handle, fb_gpu_stream_t stream,
    ...
    rocblas_context_t* ctx = (rocblas_context_t*)handle;
    rocblas_operation opA = (rocblas_operation)rocblas_convert_transpose(transa);
    rocblas_operation opB = (rocblas_operation)rocblas_convert_transpose(transb);
...
static void rocblas_ssymm_impl(void* handle, fb_gpu_stream_t stream,
                               ...
                               fb_gpu_ptr_t c, int ldc) {
    rocblas_context_t* ctx = (rocblas_context_t*)handle;
    rocblas_side side_mode = (rocblas_side)rocblas_convert_side(side);
    rocblas_fill fill = (rocblas_fill)rocblas_convert_uplo(uplo);
...
static void rocblas_strmm_impl(void* handle, fb_gpu_stream_t stream,
    ...
    rocblas_side side_mode = (rocblas_side)rocblas_convert_side(side);
    rocblas_fill fill = (rocblas_fill)rocblas_convert_uplo(uplo);
    rocblas_operation op = (rocblas_operation)rocblas_convert_transpose(transa);
    rocblas_diagonal diag_type = (rocblas_diagonal)rocblas_convert_diag(diag);
...
static int rocblas_sgetrf_impl(void* handle, fb_gpu_stream_t stream,
    ...
    // Perform LU factorization (rocSOLVER manages workspace internally)
...
static int rocblas_sgetrs_impl(void* handle, fb_gpu_stream_t stream, char trans,
    ...
    // rocSOLVER getrs doesn't use devInfo parameter
    status = rocsolver_sgetrs(ctx->solver_handle, op, n, nrhs,
                              (float*)a, lda, (int*)ipiv,
                              (float*)b, ldb);
...
static int rocblas_cgetrs_impl(void* handle, fb_gpu_stream_t stream, char trans,
    ...
    status = rocsolver_cgetrs(ctx->solver_handle, op, n, nrhs,
                              (rocblas_float_complex*)a, lda, (int*)ipiv,
                              (rocblas_float_complex*)b, ldb);
...
static int rocblas_zgetrs_impl(void* handle, fb_gpu_stream_t stream, char trans,
    ...
    status = rocsolver_zgetrs(ctx->solver_handle, op, n, nrhs,
                              (rocblas_double_complex*)a, lda, (int*)ipiv,
                              (rocblas_double_complex*)b, ldb);
...
static int rocblas_spotrf_impl(void* handle, fb_gpu_stream_t stream,
    ...
    status = rocsolver_spotrf(ctx->solver_handle, fill, n, (float*)a, lda, devInfo);
...
static int rocblas_dpotrf_impl(void* handle, fb_gpu_stream_t stream,
    ...
    status = rocsolver_dpotrf(ctx->solver_handle, fill, n, (double*)a, lda, devInfo);
...
static int rocblas_cpotrf_impl(void* handle, fb_gpu_stream_t stream,
    ...
    status = rocsolver_cpotrf(ctx->solver_handle, fill, n, (rocblas_float_complex*)a, lda, devInfo);
...
static int rocblas_zpotrf_impl(void* handle, fb_gpu_stream_t stream,
    ...
    status = rocsolver_zpotrf(ctx->solver_handle, fill, n, (rocblas_double_complex*)a, lda, devInfo);
...
static int rocblas_sgesvd_impl(void* handle, fb_gpu_stream_t stream,
    ...
    // Convert char to rocblas_svect
    rocblas_svect svect_u = (jobu == 'A' || jobu == 'a') ? rocblas_svect_all : 
                            (jobu == 'S' || jobu == 's') ? rocblas_svect_singular : rocblas_svect_none;
    rocblas_svect svect_vt = (jobvt == 'A' || jobvt == 'a') ? rocblas_svect_all :
                             (jobvt == 'S' || jobvt == 's') ? rocblas_svect_singular : rocblas_svect_none;
    
    // Allocate devInfo and E (superdiagonal)
    int minmn = (m < n) ? m : n;
    float* E;
    hipMalloc((void**)&devInfo, sizeof(int));
    hipMalloc((void**)&E, (minmn - 1) * sizeof(float));
    
    status = rocsolver_sgesvd(ctx->solver_handle, svect_u, svect_vt, m, n,
                              (float*)a, lda, (float*)s,
                              (float*)u, ldu, (float*)vt, ldvt,
                              E, rocblas_outofplace, devInfo);
...
static int rocblas_dgesvd_impl(void* handle, fb_gpu_stream_t stream,
    ...
    double* E;
    hipMalloc((void**)&devInfo, sizeof(int));
    hipMalloc((void**)&E, (minmn - 1) * sizeof(double));
    
    status = rocsolver_dgesvd(ctx->solver_handle, svect_u, svect_vt, m, n,
...
static int rocblas_cgesvd_impl(void* handle, fb_gpu_stream_t stream,
    ...
    // Allocate devInfo and E (superdiagonal - real for complex gesvd)
    int minmn = (m < n) ? m : n;
    float* E;
    hipMalloc((void**)&devInfo, sizeof(int));
    hipMalloc((void**)&E, (minmn - 1) * sizeof(float));
    
    status = rocsolver_cgesvd(ctx->solver_handle, svect_u, svect_vt, m, n,
...
static int rocblas_zgesvd_impl(void* handle, fb_gpu_stream_t stream,
    ...
    status = rocsolver_zgesvd(ctx->solver_handle, svect_u, svect_vt, m, n,
...
static int rocblas_ssyev_impl(void* handle, fb_gpu_stream_t stream,
    ...
    // Allocate devInfo and E (tridiagonal off-diagonal)
    float* E;
    hipMalloc((void**)&devInfo, sizeof(int));
    hipMalloc((void**)&E, (n - 1) * sizeof(float));
    
    status = rocsolver_ssyevd(ctx->solver_handle, jobmode, fill, n,
                              (float*)a, lda, (float*)w, E, devInfo);
...
static int rocblas_dsyev_impl(void* handle, fb_gpu_stream_t stream,
    ...
    double* E;
    hipMalloc((void**)&devInfo, sizeof(int));
    hipMalloc((void**)&E, (n - 1) * sizeof(double));
    
    status = rocsolver_dsyevd(ctx->solver_handle, jobmode, fill, n,
                              (double*)a, lda, (double*)w, E, devInfo);
...
static int rocblas_cheev_impl(void* handle, fb_gpu_stream_t stream,
    ...
    // Allocate devInfo and E (tridiagonal off-diagonal - real for hermitian)
    float* E;
    hipMalloc((void**)&devInfo, sizeof(int));
    hipMalloc((void**)&E, (n - 1) * sizeof(float));
    
    status = rocsolver_cheevd(ctx->solver_handle, jobmode, fill, n,
                              (rocblas_float_complex*)a, lda, (float*)w, E, devInfo);
...
static int rocblas_zheev_impl(void* handle, fb_gpu_stream_t stream,
    ...
    status = rocsolver_zheevd(ctx->solver_handle, jobmode, fill, n,
                              (rocblas_double_complex*)a, lda, (double*)w, E, devInfo);
