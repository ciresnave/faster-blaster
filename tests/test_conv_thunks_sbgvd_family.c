...
#define DEFINE_SBGVD_TESTS(SUFFIX, TYPE, OP_ID, BASE)                            \
...
{                                                                                 \
    ...
    int expected_lwork = 37;                                                     \
    int expected_liwork = 18;                                                    \
    int info = 0;                                                                 \
    ...
    if (info != 0 || g_##SUFFIX##_fortran_call.called != 1 ||                     \
        ...
        !g_##SUFFIX##_fortran_call.work_nonnull || g_##SUFFIX##_fortran_call.lwork != expected_lwork || \
        !g_##SUFFIX##_fortran_call.iwork_nonnull || g_##SUFFIX##_fortran_call.liwork != expected_liwork || \
        ...
        fprintf(stderr, "[FAIL] " #SUFFIX " Fortran->CBLAS thunk did not translate generalized band storage or allocate hidden work correctly\n"); \
        return 1;                                                                 \
    }                                                                             \
    printf("[PASS] " #SUFFIX " Fortran->CBLAS thunk translates A/B band storage, allocates work, and copies back eigenvectors\n"); \
    return 0;                                                                     \
}                                                                                 \
...
{                                                                                 \
    ...
    thunk(&jobz, &uplo, &n, &ka, &kb, ab, &ldab, bb, &ldbb, w, z, &ldz, work, &lwork, iwork, &liwork, &info); \
    if (g_##SUFFIX##_cblas_call.called != 1 ||                                    \
        ...
        fprintf(stderr, "[FAIL] " #SUFFIX " CBLAS->Fortran thunk did not forward SBGVD arguments correctly\n"); \
        return 1;                                                                 \
    }                                                                             \
    printf("[PASS] " #SUFFIX " CBLAS->Fortran thunk forwards SBGVD arguments\n"); \
    return 0;                                                                     \
}

DEFINE_SBGVD_TESTS(ssbgvd, float, FB_OP_SSBGVD, 100)
DEFINE_SBGVD_TESTS(dsbgvd, double, FB_OP_DSBGVD, 300)

int main(void)
{
    int status = 0;

    status |= check_ssbgvd_fortran_to_cblas();
    status |= check_ssbgvd_cblas_to_fortran();
    status |= check_dsbgvd_fortran_to_cblas();
    status |= check_dsbgvd_cblas_to_fortran();
