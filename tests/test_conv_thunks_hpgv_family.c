...
#define DEFINE_HPGV_COMPLEX_TESTS(SUFFIX, CTYPE, RTYPE, OP_ID, BASE, MAKE)      \
...
{                                                                                 \
    ...
    if (info != 0 || g_##SUFFIX##_fortran_call.called != 1 ||                    \
        ...
        fprintf(stderr, "[FAIL] " #SUFFIX " Fortran->CBLAS thunk did not translate packed HPGV state correctly\n"); \
        return 1;                                                                 \
    }                                                                             \
    printf("[PASS] " #SUFFIX " Fortran->CBLAS thunk translates packed Hermitian A/B storage and copies back eigenvectors\n"); \
    return 0;                                                                     \
}                                                                                 \
...
{                                                                                 \
    ...
    CTYPE z[1] = { MAKE(0, 0) };                                                  \
    int ldz = 1;                                                                  \
    CTYPE work[3] = { MAKE(0, 0), MAKE(0, 0), MAKE(0, 0) };                      \
    ...
    thunk(&itype, &jobz, &uplo, &n, ap, bp, w, z, &ldz, work, rwork, &info);     \
    if (g_##SUFFIX##_cblas_call.called != 1 ||                                    \
        ...
        w[0] != (RTYPE)((BASE) + 4) ||                                            \
        z[0] != MAKE((BASE) + 5, (BASE) + 15)) {                                 \
        fprintf(stderr, "[FAIL] " #SUFFIX " CBLAS->Fortran thunk did not forward HPGV arguments correctly\n"); \
        return 1;                                                                 \
    }                                                                             \
    printf("[PASS] " #SUFFIX " CBLAS->Fortran thunk forwards HPGV arguments\n"); \
    return 0;                                                                     \
}

DEFINE_HPGV_COMPLEX_TESTS(chpgv, fb_complex_float_t, float, FB_OP_CHPGV, 100, make_cfloat)
DEFINE_HPGV_COMPLEX_TESTS(zhpgv, fb_complex_double_t, double, FB_OP_ZHPGV, 300, make_cdouble)

int main(void)
{
    int status = 0;

    status |= check_chpgv_fortran_to_cblas();
    status |= check_chpgv_cblas_to_fortran();
    status |= check_zhpgv_fortran_to_cblas();
    status |= check_zhpgv_cblas_to_fortran();
