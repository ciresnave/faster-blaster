...
#define DEFINE_HPEVD_COMPLEX_TESTS(SUFFIX, CTYPE, RTYPE, OP_ID, BASE, MAKE)     \
...
typedef void (*fb_##SUFFIX##_fortran_fn)(char *jobz, char *uplo, int *n,       \
                                         ...
                                         int *ldz, CTYPE *work, int *lwork,    \
                                         RTYPE *rwork, int *lrwork, int *iwork, \
                                         int *liwork, int *info);              \
static struct {                                                                   \
    int query_calls;                                                              \
    int calls;                                                                    \
...
{                                                                                 \
    ...
    if (info != 0 || g_##SUFFIX##_fortran_call.query_calls != 1 ||               \
        ...
        fprintf(stderr, "[FAIL] " #SUFFIX " Fortran->CBLAS thunk did not translate packed HPEVD state correctly\n"); \
        return 1;                                                                 \
    }                                                                             \
    printf("[PASS] " #SUFFIX " Fortran->CBLAS thunk queries work, translates packed Hermitian storage, and copies back eigenvectors\n"); \
    return 0;                                                                     \
}                                                                                 \
...
{                                                                                 \
    ...
    thunk(&jobz, &uplo, &n, ap, w, z, &ldz, work, &lwork, rwork, &lrwork, iwork, &liwork, &info); \
    if (g_##SUFFIX##_cblas_call.called != 1 ||                                    \
        ...
        w[0] != (RTYPE)((BASE) + 4) ||                                            \
        z[0] != MAKE((BASE) + 5, (BASE) + 15)) {                                 \
        fprintf(stderr, "[FAIL] " #SUFFIX " CBLAS->Fortran thunk did not forward HPEVD arguments correctly\n"); \
        return 1;                                                                 \
    }                                                                             \
    printf("[PASS] " #SUFFIX " CBLAS->Fortran thunk forwards HPEVD arguments\n"); \
    return 0;                                                                     \
}

DEFINE_HPEVD_COMPLEX_TESTS(chpevd, fb_complex_float_t, float, FB_OP_CHPEVD, 100, make_cfloat)
DEFINE_HPEVD_COMPLEX_TESTS(zhpevd, fb_complex_double_t, double, FB_OP_ZHPEVD, 300, make_cdouble)

int main(void)
{
    int status = 0;

    status |= check_chpevd_fortran_to_cblas();
    status |= check_chpevd_cblas_to_fortran();
    status |= check_zhpevd_fortran_to_cblas();
    status |= check_zhpevd_cblas_to_fortran();
