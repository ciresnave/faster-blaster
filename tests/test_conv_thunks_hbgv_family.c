...
#define DEFINE_HBGV_TESTS(SUFFIX, TYPE, REAL_TYPE, OP_ID, BASE, MAKE, REAL_PART) \
...
typedef void (*fb_##SUFFIX##_fortran_fn)(char *jobz, char *uplo, int *n, int *ka, \
                                         ...
                                         TYPE *work, REAL_TYPE *rwork, int *info);\
static struct {                                                                    \
    int called;                                                                    \
    char jobz;                                                                     \
    char uplo;                                                                     \
...
{                                                                                  \
    ...
    if (info != 0 || g_##SUFFIX##_fortran_call.called != 1 ||                     \
        ...
        fprintf(stderr, "[FAIL] " #SUFFIX " Fortran->CBLAS thunk did not route HBGV calls\n"); \
        return 1;                                                                  \
    }                                                                              \
    for (index = 0; index < 6; ++index) {                                         \
        if (g_##SUFFIX##_fortran_call.ab_snapshot[index] != expected_ab_in[index] || \
            ...
            fprintf(stderr, "[FAIL] " #SUFFIX " Fortran->CBLAS thunk did not translate HBGV band storage correctly\n"); \
            return 1;                                                              \
        }                                                                          \
    }                                                                              \
    for (index = 0; index < 3; ++index) {                                         \
        if (w[index] != (REAL_TYPE)((BASE) + 1 + (int)index) ||                   \
            ...
            fprintf(stderr, "[FAIL] " #SUFFIX " Fortran->CBLAS thunk did not copy back HBGV eigen data correctly\n"); \
            return 1;                                                              \
        }                                                                          \
    }                                                                              \
    printf("[PASS] " #SUFFIX " Fortran->CBLAS thunk translates A/B Hermitian band storage and copies back eigenvectors\n"); \
    return 0;                                                                      \
}                                                                                  \
...
{                                                                                  \
    ...
    thunk(&jobz, &uplo, &n, &ka, &kb, ab, &ldab, bb, &ldbb, w, z, &ldz, work, rwork, &info); \
    if (g_##SUFFIX##_cblas_call.called != 1 ||                                     \
        ...
        fprintf(stderr, "[FAIL] " #SUFFIX " CBLAS->Fortran thunk did not forward HBGV arguments correctly\n"); \
        return 1;                                                                  \
    }                                                                              \
    printf("[PASS] " #SUFFIX " CBLAS->Fortran thunk forwards HBGV arguments\n"); \
    return 0;                                                                      \
}

DEFINE_HBGV_TESTS(chbgv, fb_complex_float_t, float, FB_OP_CHBGV, 100, make_cfloat, cfloat_real)
DEFINE_HBGV_TESTS(zhbgv, fb_complex_double_t, double, FB_OP_ZHBGV, 300, make_cdouble, cdouble_real)

int main(void)
{
    int status = 0;

    status |= check_chbgv_fortran_to_cblas();
    status |= check_chbgv_cblas_to_fortran();
    status |= check_zhbgv_fortran_to_cblas();
    status |= check_zhbgv_cblas_to_fortran();
