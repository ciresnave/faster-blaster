...
#define DEFINE_SPEV_TESTS(SUFFIX, TYPE, OP_ID, BASE)                             \
...
{                                                                                 \
    ...
    if (info != 0 || g_##SUFFIX##_fortran_call.called != 1 ||                    \
        ...
        fprintf(stderr, "[FAIL] " #SUFFIX " Fortran->CBLAS thunk did not translate packed SPEV state correctly\n"); \
        return 1;                                                                 \
    }                                                                             \
    printf("[PASS] " #SUFFIX " Fortran->CBLAS thunk translates packed storage and copies back eigenvectors\n"); \
    return 0;                                                                     \
}                                                                                 \
...
{                                                                                 \
    ...
    thunk(&jobz, &uplo, &n, ap, w, z, &ldz, work, &info);                        \
    if (g_##SUFFIX##_cblas_call.called != 1 ||                                    \
        ...
        fprintf(stderr, "[FAIL] " #SUFFIX " CBLAS->Fortran thunk did not forward SPEV arguments correctly\n"); \
        return 1;                                                                 \
    }                                                                             \
    printf("[PASS] " #SUFFIX " CBLAS->Fortran thunk forwards SPEV arguments\n"); \
    return 0;                                                                     \
}

DEFINE_SPEV_TESTS(sspev, float, FB_OP_SSPEV, 100)
DEFINE_SPEV_TESTS(dspev, double, FB_OP_DSPEV, 300)

int main(void)
{
    int status = 0;

    status |= check_sspev_fortran_to_cblas();
    status |= check_sspev_cblas_to_fortran();
    status |= check_dspev_fortran_to_cblas();
    status |= check_dspev_cblas_to_fortran();
