...
#define DEFINE_PPCON_REAL_TESTS(SUFFIX, TYPE, OP_ID, BASE)                       \
typedef int (*fb_##SUFFIX##_cblas_fn)(fb_layout_t layout, fb_uplo_t uplo,      \
                                      int n, const TYPE *ap, TYPE anorm,        \
                                      TYPE *rcond);                              \
...
static int stub_##SUFFIX##_cblas(fb_layout_t layout, fb_uplo_t uplo, int n,     \
                                 const TYPE *ap, TYPE anorm, TYPE *rcond)       \
{                                                                                 \
...
{                                                                                 \
    ...
    info = thunk(FB_LAYOUT_ROW_MAJOR, FB_UPPER, 3, ap_row, (TYPE)8, &rcond);    \
    if (info != (BASE) + 2 ||                                                     \
        g_##SUFFIX##_fortran_call.called != 1 ||                                  \
        ...
        rcond != (TYPE)((BASE) + 1)) {                                            \
        fprintf(stderr, "[FAIL] " #SUFFIX " Fortran->CBLAS thunk did not route packed PPCON inputs correctly\n"); \
        return 1;                                                                 \
    }                                                                             \
    printf("[PASS] " #SUFFIX " Fortran->CBLAS thunk routes packed PPCON inputs\n"); \
    return 0;                                                                     \
}                                                                                 \
...
{                                                                                 \
    ...
    thunk(&uplo, &n, ap, &anorm, &rcond, work, iwork, &info);                    \
    if (g_##SUFFIX##_cblas_call.called != 1 ||                                    \
        ...
        info != (BASE) + 4 ||                                                     \
        rcond != (TYPE)((BASE) + 3)) {                                            \
        fprintf(stderr, "[FAIL] " #SUFFIX " CBLAS->Fortran thunk did not propagate PPCON info correctly\n"); \
        return 1;                                                                 \
    }                                                                             \
    printf("[PASS] " #SUFFIX " CBLAS->Fortran thunk propagates PPCON info\n"); \
    return 0;                                                                     \
}

#define DEFINE_PPCON_COMPLEX_TESTS(SUFFIX, CTYPE, RTYPE, OP_ID, BASE)            \
typedef int (*fb_##SUFFIX##_cblas_fn)(fb_layout_t layout, fb_uplo_t uplo,      \
                                      int n, const CTYPE *ap, RTYPE anorm,      \
                                      RTYPE *rcond);                             \
...
static int stub_##SUFFIX##_cblas(fb_layout_t layout, fb_uplo_t uplo, int n,     \
                                 const CTYPE *ap, RTYPE anorm, RTYPE *rcond)    \
{                                                                                 \
    g_##SUFFIX##_cblas_call.called += 1;                                          \
    g_##SUFFIX##_cblas_call.layout = layout;                                      \
    g_##SUFFIX##_cblas_call.uplo = uplo;                                          \
...
{                                                                                 \
    ...
    info = thunk(FB_LAYOUT_COL_MAJOR, FB_UPPER, 2, ap, (RTYPE)8, &rcond);        \
    if (info != (BASE) + 2 ||                                                     \
        g_##SUFFIX##_fortran_call.called != 1 ||                                  \
        g_##SUFFIX##_fortran_call.uplo != 'U' ||                                  \
        ...
        rcond != (RTYPE)((BASE) + 1)) {                                           \
        fprintf(stderr, "[FAIL] " #SUFFIX " Fortran->CBLAS thunk did not route complex PPCON inputs correctly\n"); \
        return 1;                                                                 \
    }                                                                             \
    printf("[PASS] " #SUFFIX " Fortran->CBLAS thunk routes complex PPCON inputs\n"); \
    return 0;                                                                     \
}                                                                                 \
static int check_##SUFFIX##_cblas_to_fortran(void)                                \
{                                                                                 \
    ...
    thunk(&uplo, &n, ap, &anorm, &rcond, work, rwork, &info);                    \
    if (g_##SUFFIX##_cblas_call.called != 1 ||                                    \
        g_##SUFFIX##_cblas_call.layout != FB_LAYOUT_COL_MAJOR ||                  \
        g_##SUFFIX##_cblas_call.uplo != FB_LOWER ||                               \
        g_##SUFFIX##_cblas_call.n != 2 ||                                         \
        ...
        info != (BASE) + 4 ||                                                     \
        rcond != (RTYPE)((BASE) + 3)) {                                           \
        fprintf(stderr, "[FAIL] " #SUFFIX " CBLAS->Fortran thunk did not propagate complex PPCON info correctly\n"); \
        return 1;                                                                 \
    }                                                                             \
    printf("[PASS] " #SUFFIX " CBLAS->Fortran thunk propagates complex PPCON info\n"); \
    return 0;                                                                     \
}

DEFINE_PPCON_REAL_TESTS(sppcon, float, FB_OP_SPPCON, 100)
DEFINE_PPCON_REAL_TESTS(dppcon, double, FB_OP_DPPCON, 300)
DEFINE_PPCON_COMPLEX_TESTS(cppcon, fb_complex_float_t, float, FB_OP_CPPCON, 500)
DEFINE_PPCON_COMPLEX_TESTS(zppcon, fb_complex_double_t, double, FB_OP_ZPPCON, 700)

int main(void)
{
    int status = 0;

    status |= check_sppcon_fortran_to_cblas();
    status |= check_sppcon_cblas_to_fortran();
    status |= check_dppcon_fortran_to_cblas();
    status |= check_dppcon_cblas_to_fortran();
    status |= check_cppcon_fortran_to_cblas();
    status |= check_cppcon_cblas_to_fortran();
    status |= check_zppcon_fortran_to_cblas();
    status |= check_zppcon_cblas_to_fortran();
