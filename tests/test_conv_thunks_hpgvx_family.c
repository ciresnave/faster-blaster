...
#define DEFINE_HPGVX_COMPLEX_TESTS(SUFFIX, CTYPE, RTYPE, OP_ID, BASE, MAKE)     \
typedef int (*fb_##SUFFIX##_cblas_fn)(fb_layout_t layout, int itype, char jobz, \
                                      char range, fb_uplo_t uplo, int n,       \
                                      CTYPE *ap, CTYPE *bp, RTYPE vl,          \
                                      RTYPE vu, int il, int iu, RTYPE abstol,  \
                                      int *m, RTYPE *w, CTYPE *z, int ldz,     \
                                      int *ifail);                             \
typedef void (*fb_##SUFFIX##_fortran_fn)(int *itype, char *jobz, char *range,   \
                                         char *uplo, int *n, CTYPE *ap,        \
                                         CTYPE *bp, RTYPE *vl, RTYPE *vu,      \
                                         int *il, int *iu, RTYPE *abstol,      \
                                         int *m, RTYPE *w, CTYPE *z, int *ldz, \
                                         CTYPE *work, RTYPE *rwork, int *iwork, \
                                         int *ifail, int *info);               \
static struct {                                                                   \
    int called;                                                                   \
    int itype;                                                                    \
...
static void stub_##SUFFIX##_fortran(int *itype, char *jobz, char *range,       \
                                    char *uplo, int *n, CTYPE *ap, CTYPE *bp,  \
                                    RTYPE *vl, RTYPE *vu, int *il, int *iu,    \
                                    RTYPE *abstol, int *m, RTYPE *w, CTYPE *z, \
...
{                                                                                 \
    ...
    memcpy(g_##SUFFIX##_fortran_call.bp_snapshot, bp, elems * sizeof(CTYPE));    \
    ...
    bp[0] = MAKE(71, 21); bp[1] = MAKE(72, 22); bp[2] = MAKE(82, 24);            \
    bp[3] = MAKE(73, 23); bp[4] = MAKE(83, 25); bp[5] = MAKE(93, 26);            \
    *m = 2;                                                                       \
...
static int stub_##SUFFIX##_cblas(fb_layout_t layout, int itype, char jobz,     \
                                 char range, fb_uplo_t uplo, int n, CTYPE *ap, \
                                 CTYPE *bp, RTYPE vl, RTYPE vu, int il, int iu, \
                                 RTYPE abstol, int *m, RTYPE *w, CTYPE *z,    \
                                 int ldz, int *ifail)                          \
{                                                                                 \
...
{                                                                                 \
    ...
    CTYPE bp_row[6] = { MAKE(51, 1), MAKE(52, 2), MAKE(53, 3),                   \
                        MAKE(62, 4), MAKE(63, 5), MAKE(73, 6) };                 \
    CTYPE expected_ap_in[6] = { MAKE(11, 1), MAKE(12, 2), MAKE(22, 4),           \
                                MAKE(13, 3), MAKE(23, 5), MAKE(33, 6) };         \
    CTYPE expected_bp_in[6] = { MAKE(51, 1), MAKE(52, 2), MAKE(62, 4),           \
                                MAKE(53, 3), MAKE(63, 5), MAKE(73, 6) };         \
    CTYPE expected_ap_out[6] = { MAKE(41, 11), MAKE(42, 12), MAKE(43, 13),       \
                                 MAKE(52, 14), MAKE(53, 15), MAKE(63, 16) };     \
    CTYPE expected_bp_out[6] = { MAKE(71, 21), MAKE(72, 22), MAKE(73, 23),       \
                                 MAKE(82, 24), MAKE(83, 25), MAKE(93, 26) };     \
    RTYPE w[3] = { 0, 0, 0 };                                                     \
    ...
    info = thunk(FB_LAYOUT_ROW_MAJOR, 3, 'V', 'I', FB_UPPER, 3, ap_row, bp_row, \
                 (RTYPE)0, (RTYPE)0, 1, 2, (RTYPE)0.5, &m, w, z, 2, ifail);     \
    if (info != 0 || g_##SUFFIX##_fortran_call.called != 1 ||                    \
        ...
        fprintf(stderr, "[FAIL] " #SUFFIX " Fortran->CBLAS thunk did not translate packed HPGVX state correctly\n"); \
        return 1;                                                                 \
    }                                                                             \
    printf("[PASS] " #SUFFIX " Fortran->CBLAS thunk translates packed Hermitian A/B storage and copies back selected eigenvectors\n"); \
    return 0;                                                                     \
}                                                                                 \
...
{                                                                                 \
    ...
    CTYPE ap[6] = { MAKE(7, 1), MAKE(8, 2), MAKE(9, 3), MAKE(10, 4),            \
                    MAKE(11, 5), MAKE(12, 6) };                                  \
    CTYPE bp[6] = { MAKE(13, 1), MAKE(14, 2), MAKE(15, 3), MAKE(16, 4),         \
                    MAKE(17, 5), MAKE(18, 6) };                                  \
    RTYPE vl = (RTYPE)1.5;                                                        \
    RTYPE vu = (RTYPE)2.5;                                                        \
    int il = 1;                                                                   \
    ...
    thunk(&itype, &jobz, &range, &uplo, &n, ap, bp, &vl, &vu, &il, &iu, &abstol, \
    ...
    if (g_##SUFFIX##_cblas_call.called != 1 ||                                    \
        ...
        fprintf(stderr, "[FAIL] " #SUFFIX " CBLAS->Fortran thunk did not forward HPGVX arguments correctly\n"); \
        return 1;                                                                 \
    }                                                                             \
    printf("[PASS] " #SUFFIX " CBLAS->Fortran thunk forwards HPGVX arguments\n"); \
    return 0;                                                                     \
}

DEFINE_HPGVX_COMPLEX_TESTS(chpgvx, fb_complex_float_t, float, FB_OP_CHPGVX, 100, make_cfloat)
DEFINE_HPGVX_COMPLEX_TESTS(zhpgvx, fb_complex_double_t, double, FB_OP_ZHPGVX, 300, make_cdouble)

int main(void)
{
    int status = 0;

    status |= check_chpgvx_fortran_to_cblas();
    status |= check_chpgvx_cblas_to_fortran();
    status |= check_zhpgvx_fortran_to_cblas();
    status |= check_zhpgvx_cblas_to_fortran();
