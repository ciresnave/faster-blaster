...
typedef int (*fb_dpbsv_fn)(fb_layout_t layout, fb_uplo_t uplo, int n, int kd,
...
typedef int (*fb_zpbsv_fn)(fb_layout_t layout, fb_uplo_t uplo, int n, int kd,
...
typedef void (*fb_spbsv_fortran_slot_fn)(char *uplo, int *n, int *kd, int *nrhs,
                                         ...
                                         int *ldb, int *info);
typedef void (*fb_dpbsv_fortran_slot_fn)(char *uplo, int *n, int *kd, int *nrhs,
...
typedef void (*fb_cpbsv_fortran_slot_fn)(char *uplo, int *n, int *kd, int *nrhs,
                                         ...
                                         fb_complex_float_t *b, int *ldb,
                                         int *info);
typedef void (*fb_zpbsv_fortran_slot_fn)(char *uplo, int *n, int *kd, int *nrhs,
...
static void stub_dpbsv_fortran(char *uplo, int *n, int *kd, int *nrhs,
                               double *ab, int *ldab, double *b, int *ldb,
                               int *info)
{
    static const double factor_ab_col[8] = { 0.0, 220.0, 211.0, 221.0,
                                             212.0, 222.0, 213.0, 223.0 };
    static const double solution_b_col[8] = { 110.0, 130.0, 150.0, 170.0,
                                              120.0, 140.0, 160.0, 180.0 };
    int index = 0;

    g_dpbsv_fortran_call.calls += 1;
    g_dpbsv_fortran_call.uplo = *uplo;
    g_dpbsv_fortran_call.n = *n;
    g_dpbsv_fortran_call.kd = *kd;
    g_dpbsv_fortran_call.nrhs = *nrhs;
    g_dpbsv_fortran_call.ldab = *ldab;
    g_dpbsv_fortran_call.ldb = *ldb;
    for (index = 0; index < (*ldab * *n); ++index) {
        g_dpbsv_fortran_call.ab_snapshot[index] = ab[index];
    }
    for (index = 0; index < (*ldb * *nrhs); ++index) {
        g_dpbsv_fortran_call.b_snapshot[index] = b[index];
    }
    memcpy(ab, factor_ab_col, sizeof(factor_ab_col));
    memcpy(b, solution_b_col, sizeof(solution_b_col));
    *info = 0;
...
static int stub_dpbsv_cblas(fb_layout_t layout, fb_uplo_t uplo, int n, int kd,
                            int nrhs, double *ab, int ldab, double *b, int ldb)
{
    g_dpbsv_cblas_call.called += 1;
    g_dpbsv_cblas_call.layout = layout;
    g_dpbsv_cblas_call.uplo = uplo;
    g_dpbsv_cblas_call.n = n;
    g_dpbsv_cblas_call.kd = kd;
    g_dpbsv_cblas_call.nrhs = nrhs;
    g_dpbsv_cblas_call.ldab = ldab;
    g_dpbsv_cblas_call.ldb = ldb;
    g_dpbsv_cblas_call.ab = ab;
    g_dpbsv_cblas_call.b = b;
    b[0] = 199.0;
    return 162;
}
...
static void stub_zpbsv_fortran(char *uplo, int *n, int *kd, int *nrhs,
                               fb_complex_double_t *ab, int *ldab,
                               fb_complex_double_t *b, int *ldb, int *info)
{
    static const double factor_ab_col_real[8] = { 330.0, 430.0, 331.0, 431.0,
                                                  332.0, 432.0, 333.0, 0.0 };
    static const double solution_b_col_real[8] = { 310.0, 330.0, 350.0, 370.0,
                                                   320.0, 340.0, 360.0, 380.0 };
    int index = 0;

    g_zpbsv_fortran_call.calls += 1;
    g_zpbsv_fortran_call.uplo = *uplo;
    g_zpbsv_fortran_call.n = *n;
    g_zpbsv_fortran_call.kd = *kd;
    g_zpbsv_fortran_call.nrhs = *nrhs;
    g_zpbsv_fortran_call.ldab = *ldab;
    g_zpbsv_fortran_call.ldb = *ldb;
    for (index = 0; index < (*ldab * *n); ++index) {
        g_zpbsv_fortran_call.ab_real_snapshot[index] = cdouble_real(ab[index]);
    }
    for (index = 0; index < (*ldb * *nrhs); ++index) {
        g_zpbsv_fortran_call.b_real_snapshot[index] = cdouble_real(b[index]);
    }
    for (index = 0; index < 8; ++index) {
...
static int stub_zpbsv_cblas(fb_layout_t layout, fb_uplo_t uplo, int n, int kd,
                            int nrhs, fb_complex_double_t *ab, int ldab,
                            fb_complex_double_t *b, int ldb)
{
    g_zpbsv_cblas_call.called += 1;
    g_zpbsv_cblas_call.layout = layout;
    g_zpbsv_cblas_call.uplo = uplo;
    g_zpbsv_cblas_call.n = n;
    g_zpbsv_cblas_call.kd = kd;
    g_zpbsv_cblas_call.nrhs = nrhs;
    g_zpbsv_cblas_call.ldab = ldab;
    g_zpbsv_cblas_call.ldb = ldb;
    g_zpbsv_cblas_call.ab = ab;
    g_zpbsv_cblas_call.b = b;
    b[0] = make_cdouble(399.0);
    return 164;
}
...
static int check_dpbsv_fortran_to_cblas(void)
{
    fb_backend_vtable_t vtable;
    fb_dpbsv_fn thunk = NULL;
    ...
    double expected_ab_out[8] = { 0.0, 211.0, 212.0, 213.0, 220.0, 221.0, 222.0, 223.0 };
    double expected_b_out[8] = { 110.0, 120.0, 130.0, 140.0, 150.0, 160.0, 170.0, 180.0 };
    int info = 0;

    memset(&vtable, 0, sizeof(vtable));
    memset(&g_dpbsv_fortran_call, 0, sizeof(g_dpbsv_fortran_call));

    vtable.ext_ops[FB_OP_DPBSV][FB_CONV_FORTRAN] =
        (fb_generic_fn)(void (*)(void))stub_dpbsv_fortran;
    fb_install_conv_thunks(&vtable, FB_OP_DPBSV);

    thunk = (fb_dpbsv_fn)vtable.ext_ops[FB_OP_DPBSV][FB_CONV_CBLAS];
    if (!thunk) {
        fprintf(stderr, "[FAIL] DPBSV Fortran->CBLAS thunk was not installed\n");
        return 1;
    }

    info = thunk(FB_LAYOUT_ROW_MAJOR, FB_UPPER, 4, 1, 2, ab, 4, b, 2);
    if (info != 0 || g_dpbsv_fortran_call.calls != 1 ||
        g_dpbsv_fortran_call.uplo != 'U' || g_dpbsv_fortran_call.n != 4 ||
        g_dpbsv_fortran_call.kd != 1 || g_dpbsv_fortran_call.nrhs != 2 ||
        g_dpbsv_fortran_call.ldab != 2 || g_dpbsv_fortran_call.ldb != 4 ||
        memcmp(g_dpbsv_fortran_call.ab_snapshot, expected_ab_in, sizeof(expected_ab_in)) != 0 ||
        memcmp(g_dpbsv_fortran_call.b_snapshot, expected_b_in, sizeof(expected_b_in)) != 0 ||
        memcmp(ab, expected_ab_out, sizeof(expected_ab_out)) != 0 ||
        memcmp(b, expected_b_out, sizeof(expected_b_out)) != 0) {
        fprintf(stderr, "[FAIL] DPBSV Fortran->CBLAS thunk did not preserve band solve row-major translation and copy-back\n");
        return 1;
    }

    printf("[PASS] DPBSV Fortran->CBLAS thunk transposes band storage and RHS data, then copies both outputs back to row-major\n");
    return 0;
}

static int check_dpbsv_cblas_to_fortran(void)
{
    fb_backend_vtable_t vtable;
    fb_dpbsv_fortran_slot_fn thunk = NULL;
    ...
    memset(&vtable, 0, sizeof(vtable));
    memset(&g_dpbsv_cblas_call, 0, sizeof(g_dpbsv_cblas_call));

    vtable.ext_ops[FB_OP_DPBSV][FB_CONV_CBLAS] =
        (fb_generic_fn)(void (*)(void))stub_dpbsv_cblas;
    fb_install_conv_thunks(&vtable, FB_OP_DPBSV);

    thunk = (fb_dpbsv_fortran_slot_fn)vtable.ext_ops[FB_OP_DPBSV][FB_CONV_FORTRAN];
    if (!thunk) {
        fprintf(stderr, "[FAIL] DPBSV CBLAS->Fortran thunk was not installed\n");
        return 1;
    }

    thunk(&uplo, &n, &kd, &nrhs, ab, &ldab, b, &ldb, &info);
    if (info != 162 || g_dpbsv_cblas_call.called != 1 ||
        g_dpbsv_cblas_call.layout != FB_LAYOUT_COL_MAJOR ||
        g_dpbsv_cblas_call.uplo != FB_LOWER ||
        g_dpbsv_cblas_call.n != 3 || g_dpbsv_cblas_call.kd != 1 ||
        g_dpbsv_cblas_call.nrhs != 2 || g_dpbsv_cblas_call.ldab != 2 ||
        g_dpbsv_cblas_call.ldb != 3 || g_dpbsv_cblas_call.ab != ab ||
        g_dpbsv_cblas_call.b != b || b[0] != 199.0) {
        fprintf(stderr, "[FAIL] DPBSV CBLAS->Fortran thunk delegated incorrectly\n");
        return 1;
    }

    printf("[PASS] DPBSV CBLAS->Fortran thunk maps the all-pointer ABI into the generic double band-solve entry\n");
    return 0;
}
...
static int check_zpbsv_fortran_to_cblas(void)
{
    fb_backend_vtable_t vtable;
    fb_zpbsv_fn thunk = NULL;
    ...
    double expected_ab_in[8] = { 30.0, 40.0, 31.0, 41.0, 32.0, 42.0, 33.0, 0.0 };
    double expected_b_in[8] = { 1.0, 3.0, 5.0, 7.0, 2.0, 4.0, 6.0, 8.0 };
    double expected_ab_out[8] = { 330.0, 331.0, 332.0, 333.0, 430.0, 431.0, 432.0, 0.0 };
    double expected_b_out[8] = { 310.0, 320.0, 330.0, 340.0, 350.0, 360.0, 370.0, 380.0 };
    ...
    memset(&vtable, 0, sizeof(vtable));
    memset(&g_zpbsv_fortran_call, 0, sizeof(g_zpbsv_fortran_call));

    vtable.ext_ops[FB_OP_ZPBSV][FB_CONV_FORTRAN] =
        (fb_generic_fn)(void (*)(void))stub_zpbsv_fortran;
    fb_install_conv_thunks(&vtable, FB_OP_ZPBSV);

    thunk = (fb_zpbsv_fn)vtable.ext_ops[FB_OP_ZPBSV][FB_CONV_CBLAS];
    if (!thunk) {
        fprintf(stderr, "[FAIL] ZPBSV Fortran->CBLAS thunk was not installed\n");
        return 1;
    }
    ...
    if (info != 0 || g_zpbsv_fortran_call.calls != 1 ||
        g_zpbsv_fortran_call.uplo != 'L' || g_zpbsv_fortran_call.n != 4 ||
        g_zpbsv_fortran_call.kd != 1 || g_zpbsv_fortran_call.nrhs != 2 ||
        g_zpbsv_fortran_call.ldab != 2 || g_zpbsv_fortran_call.ldb != 4 ||
        memcmp(g_zpbsv_fortran_call.ab_real_snapshot, expected_ab_in, sizeof(expected_ab_in)) != 0 ||
        memcmp(g_zpbsv_fortran_call.b_real_snapshot, expected_b_in, sizeof(expected_b_in)) != 0 ||
        memcmp(actual_ab_out, expected_ab_out, sizeof(expected_ab_out)) != 0 ||
        memcmp(actual_b_out, expected_b_out, sizeof(expected_b_out)) != 0) {
        fprintf(stderr, "[FAIL] ZPBSV Fortran->CBLAS thunk did not preserve complex-double band solve row-major translation and copy-back\n");
        return 1;
    }

    printf("[PASS] ZPBSV Fortran->CBLAS thunk transposes Hermitian band storage and RHS data, then copies both outputs back to row-major\n");
    return 0;
}

static int check_zpbsv_cblas_to_fortran(void)
{
    fb_backend_vtable_t vtable;
    fb_zpbsv_fortran_slot_fn thunk = NULL;
    ...
    fb_complex_double_t b[6] = {
        make_cdouble(10.0), make_cdouble(20.0), make_cdouble(30.0),
        make_cdouble(40.0), make_cdouble(50.0), make_cdouble(60.0)
    };
    char uplo = 'U';
    int n = 3;
    int kd = 1;
    ...
    memset(&vtable, 0, sizeof(vtable));
    memset(&g_zpbsv_cblas_call, 0, sizeof(g_zpbsv_cblas_call));

    vtable.ext_ops[FB_OP_ZPBSV][FB_CONV_CBLAS] =
        (fb_generic_fn)(void (*)(void))stub_zpbsv_cblas;
    fb_install_conv_thunks(&vtable, FB_OP_ZPBSV);

    thunk = (fb_zpbsv_fortran_slot_fn)vtable.ext_ops[FB_OP_ZPBSV][FB_CONV_FORTRAN];
    if (!thunk) {
        fprintf(stderr, "[FAIL] ZPBSV CBLAS->Fortran thunk was not installed\n");
        return 1;
    }

    thunk(&uplo, &n, &kd, &nrhs, ab, &ldab, b, &ldb, &info);
    if (info != 164 || g_zpbsv_cblas_call.called != 1 ||
        g_zpbsv_cblas_call.layout != FB_LAYOUT_COL_MAJOR ||
        g_zpbsv_cblas_call.uplo != FB_UPPER ||
        g_zpbsv_cblas_call.n != 3 || g_zpbsv_cblas_call.kd != 1 ||
        g_zpbsv_cblas_call.nrhs != 2 || g_zpbsv_cblas_call.ldab != 2 ||
        g_zpbsv_cblas_call.ldb != 3 || g_zpbsv_cblas_call.ab != ab ||
        g_zpbsv_cblas_call.b != b || cdouble_real(b[0]) != 399.0) {
        fprintf(stderr, "[FAIL] ZPBSV CBLAS->Fortran thunk delegated incorrectly\n");
        return 1;
    }

    printf("[PASS] ZPBSV CBLAS->Fortran thunk maps the all-pointer ABI into the generic complex-double band-solve entry\n");
    return 0;
}
...
int main(void)
{
    ...
    if (check_dpbsv_fortran_to_cblas() != 0) {
        return 1;
    }
    if (check_dpbsv_cblas_to_fortran() != 0) {
        return 1;
    }
    ...
    if (check_zpbsv_fortran_to_cblas() != 0) {
        return 1;
    }
    if (check_zpbsv_cblas_to_fortran() != 0) {
        return 1;
    }
