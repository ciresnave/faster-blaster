...
static int stub_##SUFFIX##_cblas(fb_layout_t layout, int itype, char jobz,     \
                                 char range, fb_uplo_t uplo, int n, TYPE *ap,  \
                                 TYPE *bp, TYPE vl, TYPE vu, int il, int iu,   \
                                 TYPE abstol, int *m, TYPE *w, TYPE *z,        \
                                 int ldz, int *ifail)                          \
{                                                                                 \
...
{                                                                                 \
    ...
    if (info != 0 || g_##SUFFIX##_fortran_call.called != 1 ||                    \
        ...
        fprintf(stderr, "[FAIL] " #SUFFIX " Fortran->CBLAS thunk did not translate packed SPGVX state correctly\n"); \
        return 1;                                                                 \
    }                                                                             \
    printf("[PASS] " #SUFFIX " Fortran->CBLAS thunk translates packed A/B storage and copies back selected eigenvectors\n"); \
    return 0;                                                                     \
}                                                                                 \
...
{                                                                                 \
    ...
    TYPE bp[6] = { (TYPE)13, (TYPE)14, (TYPE)15, (TYPE)16, (TYPE)17, (TYPE)18 }; \
    TYPE vl = (TYPE)1.5;                                                          \
    TYPE vu = (TYPE)2.5;                                                          \
    int il = 1;                                                                   \
    int iu = 2;                                                                   \
    ...
    if (g_##SUFFIX##_cblas_call.called != 1 ||                                    \
        ...
        fprintf(stderr, "[FAIL] " #SUFFIX " CBLAS->Fortran thunk did not forward SPGVX arguments correctly\n"); \
        return 1;                                                                 \
    }                                                                             \
    printf("[PASS] " #SUFFIX " CBLAS->Fortran thunk forwards SPGVX arguments\n"); \
    return 0;                                                                     \
}

DEFINE_SPGVX_TESTS(sspgvx, float, FB_OP_SSPGVX, 100)
DEFINE_SPGVX_TESTS(dspgvx, double, FB_OP_DSPGVX, 300)

int main(void)
{
    int status = 0;

    status |= check_sspgvx_fortran_to_cblas();
    status |= check_sspgvx_cblas_to_fortran();
    status |= check_dspgvx_fortran_to_cblas();
    status |= check_dspgvx_cblas_to_fortran();
