...
#define DEFINE_HBEV_TESTS(SUFFIX, TYPE, REAL_TYPE, OP_ID, BASE, MAKE, REAL_PART) \
...
typedef void (*fb_##SUFFIX##_fortran_fn)(char *jobz, char *uplo, int *n, int *kd, \
                                         ...
                                         int *ldz, TYPE *work, REAL_TYPE *rwork, int *info); \
static struct {                                                                    \
    int called;                                                                    \
    char jobz;                                                                     \
    char uplo;                                                                     \
...
static void stub_##SUFFIX##_fortran(char *jobz, char *uplo, int *n, int *kd,    \
                                    TYPE *ab, int *ldab, REAL_TYPE *w, TYPE *z, \
                                    int *ldz, TYPE *work, REAL_TYPE *rwork,      \
                                    int *info)                                    \
{                                                                                  \
    size_t index = 0;                                                              \
...
{                                                                                  \
    ...
    TYPE ab[6] = { MAKE((REAL_TYPE)0), MAKE((REAL_TYPE)12), MAKE((REAL_TYPE)23), \
                   MAKE((REAL_TYPE)11), MAKE((REAL_TYPE)22), MAKE((REAL_TYPE)33) }; \
    REAL_TYPE expected_ab_after[6] = { (REAL_TYPE)0, (REAL_TYPE)12, (REAL_TYPE)23, \
                                       (REAL_TYPE)11, (REAL_TYPE)22, (REAL_TYPE)33 }; \
    ...
    if (info != 0 || g_##SUFFIX##_fortran_call.called != 1 ||                     \
        ...
        fprintf(stderr, "[FAIL] " #SUFFIX " Fortran->CBLAS thunk did not route HBEV calls\n"); \
        return 1;                                                                  \
    }                                                                              \
    for (index = 0; index < 6; ++index) {                                         \
        if (g_##SUFFIX##_fortran_call.ab_snapshot[index] != expected_ab_in[index] || \
            REAL_PART(ab[index]) != expected_ab_after[index]) {                   \
            fprintf(stderr, "[FAIL] " #SUFFIX " Fortran->CBLAS thunk did not translate HBEV band storage correctly\n"); \
            return 1;                                                              \
        }                                                                          \
    }                                                                              \
    for (index = 0; index < 3; ++index) {                                         \
        if (w[index] != (REAL_TYPE)((BASE) + 1 + (int)index) ||                   \
            ...
            fprintf(stderr, "[FAIL] " #SUFFIX " Fortran->CBLAS thunk did not copy back HBEV eigen data correctly\n"); \
            return 1;                                                              \
        }                                                                          \
    }                                                                              \
    printf("[PASS] " #SUFFIX " Fortran->CBLAS thunk translates Hermitian band storage and copies back eigenvectors\n"); \
    return 0;                                                                      \
}                                                                                  \
...
{                                                                                  \
    ...
    int ldab = 2;                                                                  \
    REAL_TYPE w[2] = { 0, 0 };                                                     \
    TYPE z[1] = { MAKE((REAL_TYPE)0) };                                            \
    int ldz = 1;                                                                   \
    TYPE work[2] = { MAKE((REAL_TYPE)0), MAKE((REAL_TYPE)0) };                    \
    REAL_TYPE rwork[4] = { 0, 0, 0, 0 };                                           \
    int info = -999;                                                               \
    memset(&vtable, 0, sizeof(vtable));                                            \
    ...
    thunk(&jobz, &uplo, &n, &kd, ab, &ldab, w, z, &ldz, work, rwork, &info);      \
    if (g_##SUFFIX##_cblas_call.called != 1 ||                                     \
        ...
        fprintf(stderr, "[FAIL] " #SUFFIX " CBLAS->Fortran thunk did not forward HBEV arguments correctly\n"); \
        return 1;                                                                  \
    }                                                                              \
    printf("[PASS] " #SUFFIX " CBLAS->Fortran thunk forwards HBEV arguments\n"); \
    return 0;                                                                      \
}

DEFINE_HBEV_TESTS(chbev, fb_complex_float_t, float, FB_OP_CHBEV, 100, make_cfloat, cfloat_real)
DEFINE_HBEV_TESTS(zhbev, fb_complex_double_t, double, FB_OP_ZHBEV, 300, make_cdouble, cdouble_real)

int main(void)
{
    int status = 0;

    status |= check_chbev_fortran_to_cblas();
    status |= check_chbev_cblas_to_fortran();
    status |= check_zhbev_fortran_to_cblas();
    status |= check_zhbev_cblas_to_fortran();
