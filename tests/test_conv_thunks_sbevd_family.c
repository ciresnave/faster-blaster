...
#define DEFINE_SBEVD_TESTS(SUFFIX, TYPE, OP_ID, BASE)                            \
...
typedef void (*fb_##SUFFIX##_fortran_fn)(char *jobz, char *uplo, int *n, int *kd,\
                                         ...
                                         int *ldz, TYPE *work, int *lwork, int *iwork,\
                                         int *liwork, int *info);              \
static struct {                                                                   \
    int called;                                                                   \
    char jobz;                                                                    \
...
{                                                                                 \
    ...
    if (info != 0 || g_##SUFFIX##_fortran_call.called != 1 ||                     \
        ...
        g_##SUFFIX##_fortran_call.lwork != expected_lwork ||                      \
        !g_##SUFFIX##_fortran_call.iwork_nonnull ||                                \
        g_##SUFFIX##_fortran_call.liwork != expected_liwork ||                    \
        w[0] != (TYPE)((BASE) + 1) || w[1] != (TYPE)((BASE) + 2) ||               \
        w[2] != (TYPE)((BASE) + 3) || memcmp(z, expected_z, sizeof(expected_z)) != 0) { \
        fprintf(stderr, "[FAIL] " #SUFFIX " Fortran->CBLAS thunk did not translate SBEVD storage, allocate hidden work, or copy back vectors correctly\n"); \
        return 1;                                                                 \
    }                                                                             \
    printf("[PASS] " #SUFFIX " Fortran->CBLAS thunk translates band storage, allocates work, and copies back eigenvectors\n"); \
    return 0;                                                                     \
}                                                                                 \
...
{                                                                                 \
    ...
    thunk(&jobz, &uplo, &n, &kd, ab, &ldab, w, z, &ldz, work, &lwork, iwork,     \
          &liwork, &info);                                                        \
    if (g_##SUFFIX##_cblas_call.called != 1 ||                                    \
        ...
        fprintf(stderr, "[FAIL] " #SUFFIX " CBLAS->Fortran thunk did not forward SBEVD arguments correctly\n"); \
        return 1;                                                                 \
    }                                                                             \
    printf("[PASS] " #SUFFIX " CBLAS->Fortran thunk forwards SBEVD arguments\n"); \
    return 0;                                                                     \
}

DEFINE_SBEVD_TESTS(ssbevd, float, FB_OP_SSBEVD, 100)
DEFINE_SBEVD_TESTS(dsbevd, double, FB_OP_DSBEVD, 300)

int main(void)
{
    int status = 0;

    status |= check_ssbevd_fortran_to_cblas();
    status |= check_ssbevd_cblas_to_fortran();
    status |= check_dsbevd_fortran_to_cblas();
    status |= check_dsbevd_cblas_to_fortran();
