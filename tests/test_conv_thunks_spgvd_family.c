...
#define DEFINE_SPGVD_TESTS(SUFFIX, TYPE, OP_ID, BASE)                            \
...
{                                                                                 \
    ...
    if (info != 0 || g_##SUFFIX##_fortran_call.query_calls != 1 ||               \
        ...
        fprintf(stderr, "[FAIL] " #SUFFIX " Fortran->CBLAS thunk did not translate packed SPGVD state correctly\n"); \
        return 1;                                                                 \
    }                                                                             \
    printf("[PASS] " #SUFFIX " Fortran->CBLAS thunk queries work, translates packed A/B storage, and copies back eigenvectors\n"); \
    return 0;                                                                     \
}                                                                                 \
...
{                                                                                 \
    ...
    thunk(&itype, &jobz, &uplo, &n, ap, bp, w, z, &ldz, work, &lwork, iwork, &liwork, &info); \
    if (g_##SUFFIX##_cblas_call.called != 1 ||                                   \
        ...
        fprintf(stderr, "[FAIL] " #SUFFIX " CBLAS->Fortran thunk did not forward SPGVD arguments correctly\n"); \
        return 1;                                                                 \
    }                                                                             \
    printf("[PASS] " #SUFFIX " CBLAS->Fortran thunk forwards SPGVD arguments\n"); \
    return 0;                                                                     \
}

DEFINE_SPGVD_TESTS(sspgvd, float, FB_OP_SSPGVD, 100)
DEFINE_SPGVD_TESTS(dspgvd, double, FB_OP_DSPGVD, 300)

int main(void)
{
    int status = 0;

    status |= check_sspgvd_fortran_to_cblas();
    status |= check_sspgvd_cblas_to_fortran();
    status |= check_dspgvd_fortran_to_cblas();
    status |= check_dspgvd_cblas_to_fortran();
