...
function(prompt_user PROMPT DEFAULT_VALUE OUT_VAR)
    if(INTERACTIVE_BACKEND_SETUP AND NOT DEFINED ${OUT_VAR})
        message("")
        message("${PROMPT}")
        if(DEFAULT_VALUE)
        ...
        # For automated builds, use default
        if(NOT CMAKE_INTERACTIVE)
            set(${OUT_VAR} ${DEFAULT_VALUE} PARENT_SCOPE)
            return()
        endif()
        
        # Try to read user input (CMake limitation: this doesn't work in all scenarios)
        # In practice, we'll set defaults and let users override with -D flags
        set(${OUT_VAR} ${DEFAULT_VALUE} PARENT_SCOPE)
    else()
        set(${OUT_VAR} ${DEFAULT_VALUE} PARENT_SCOPE)
    endif()
...
if(AUTO_DETECT_BACKENDS)
    ...
    if(OpenBLAS_FOUND AND NOT OPENBLAS_BUILT_FROM_SOURCE)
        if(INTERACTIVE_BACKEND_SETUP AND BUILD_OPENBLAS_FROM_SOURCE)
            message(STATUS "")
