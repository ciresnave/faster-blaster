...
$log = '.\judge_heap_validate_' + $stamp + '.txt'
$err = '.\judge_heap_validate_' + $stamp + '.err.txt'
$tail = '.\judge_heap_validate_latest_tail.txt'
$status = '.\judge_heap_validate_latest_status.txt'
...
try {
	...
	if (Test-Path $log) {
		try {
			Get-Content $log -Tail 40 | Set-Content $tail -Encoding utf8
		} catch {
	...
	if ((Test-Path $err) -and ((Get-Item $err).Length -gt 0)) {
		...
		Get-Content $err -Tail 40 | Add-Content $tail
	}
	...
	if ($code -eq 0) {
		...
		Get-Content $tail
		Write-Error ('test_judge_runner exited with code ' + $code)
		exit $code
	}
}
