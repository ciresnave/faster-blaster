/* sgbtrs - solve banded system using LU factorization (single precision) */
void sgbtrs_(const char *trans, const int *n, const int *kl, const int *ku, const int *nrhs,
             const float *AB, const int *ldab, const int *ipiv, float *B, const int *ldb, int *info)
{
    int nn = *n;
    int kl_val = *kl;
    int ku_val = *ku;
    ...
    *info = 0;
    ...
    if (trans_val == 'N' || trans_val == 'n') {
        ...
        for (int i = nn - 1; i >= 0; i--) {
            ...
            float diag = AB[kl_val + i * ldab_val];
