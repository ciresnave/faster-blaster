...
/* SLARFB - Apply block Householder reflection
   SIDE: 'L' or 'R'
   TRANS: 'N' or 'T'
   DIRECT: 'F' or 'B'
   STOREV: 'C' or 'R'
...
*/
void slarfb_(const char *side, const char *trans, const char *direct, const char *storev,
             const int *m, const int *n, const int *k, const float *v, const int *ldv,
             ...
             float *work, const int *ldwork)
{
    ...
    float tau = 1.0f;  /* Simplified: use unit tau */
    ...
    if (side[0] == 'L') {
        ...
        for (int i = 0; i < k_val; i++) {
            ...
            /* Apply with SLARF */
            slarf_(side, m, n, work_v, &m_val, &tau, c, ldc, work + m_val);
        }
    } else {
        /* Apply from right */
        for (int i = 0; i < k_val; i++) {
            ...
            /* Apply with SLARF */
            slarf_(side, m, n, work_v, &n_val, &tau, c, ldc, work + n_val);
        }
    }
}
