/* dgbtrf - banded LU factorization with partial pivoting (double precision) */
void dgbtrf_(const int *m, const int *n, const int *kl, const int *ku, double *AB, 
             const int *ldab, int *ipiv, int *info)
{
    ...
    *info = 0;
    ...
    for (int j = 0; j < minmn; j++) {
        ...
        int pivot_row = j;
        double pivot_val = fabs(AB[kl_val + j + j * ldab_val]);
        
        for (int i = j + 1; i <= irow_end; i++) {
            int band_idx = kl_val + j - i;  // Band storage row index
            if (band_idx >= 0) {
                double val = fabs(AB[band_idx + i * ldab_val]);
                if (val > pivot_val) {
                    pivot_val = val;
                    pivot_row = i;
                }
        ...
        if (pivot_row != j) {
            ...
            for (int i = i1_start; i <= i1_end; i++) {
                double temp = AB[kl_val + j - i + i * ldab_val];
        ...
        // Scale L factors
        double diag = AB[kl_val + j + j * ldab_val];
