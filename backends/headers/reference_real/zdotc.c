/**
 * @file zdotc.c
 * @brief Reference implementation of ZDOTC (Complex double precision dot product, conjugated)
 *
 * ZDOTC: sum = x^H * y = conj(x)^T * y (complex double, conjugate first vector)
 */
 ...
 * Complex conjugate
 */
static inline complex_d conj_d(complex_d a) {
    return (complex_d){a.real, -a.imag};
}
 ...
 */
complex_d zdotc_ref(int n, const complex_d *x, int incx, const complex_d *y, int incy) {
    ...
    if (incx == 1 && incy == 1) {
        for (int i = 0; i < n; i++) {
            complex_d prod = cmul_d(conj_d(x[i]), y[i]);
        ...
        for (int i = 0; i < n; i++) {
            complex_d prod = cmul_d(conj_d(x[ix]), y[iy]);
