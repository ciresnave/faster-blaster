/* dtrexc - Reorder Schur form by swapping diagonal blocks */

#

/* DTREXC - Reorder Schur form T by swapping adjacent blocks (double precision)
...
void dtrexc_(const char *compq, const int *n, double *t, const int *ldt,
             double *q, const int *ldq, int *ifst, int *ilst, double *work,
             int *info)
{
    *info = 0;
    int n_val = *n;
    ...
    for (int idx = idx_start; (dir > 0) ? (idx < idx_end) : (idx < idx_end); idx += dir) {
        ...
        if (i < j) {
            double a = t[i + i * ldt_val];
            double b = t[j + i * ldt_val];
        ...
        for (int col = i; col < n_val; col++) {
            double tmp = c * t[i + col * ldt_val] - s * t[j + col * ldt_val];
            t[j + col * ldt_val] = s * t[i + col * ldt_val] + c * t[j + col * ldt_val];
            t[i + col * ldt_val] = tmp;
        }

        for (int row = 0; row <= j; row++) {
            double tmp = c * t[row + i * ldt_val] + s * t[row + j * ldt_val];
        ...
        if (compq[0] == 'V') {
            for (int row = 0; row < n_val; row++) {
                double tmp = c * q[row + i * ldq_val] - s * q[row + j * ldq_val];
